...
async function call(path, source, fields = {}, retries = 0) {
  ...
  if (response.status === 524 && retries > 0) {
    console.error(`[${path}] origin timed out; retrying once to pick up the completed Modal cache entry`)
    await new Promise(resolve => setTimeout(resolve, 5_000))
    return call(path, source, fields, retries - 1)
  }
...
const sliced = await call("/v1/pdf/parse", local, { pages: localRange }, 1)
...
const largeSlice = await call("/v1/pdf/parse", large, { pages: "1,30,60" }, 1)
